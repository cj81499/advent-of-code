...
class Pipe(enum.StrEnum):
    ...
    def relative_connections(self) -> tuple[complex, complex]:
        match self:
...
class AnimalGridEntry(GridEntry):
    ...
    def connections(self) -> Iterable[GridEntry]:
        return (
            ge
            for p in util.adj_4(self.pos)
            if (ge := self.grid.get(p)) is not None and self.pos in (c.pos for c in ge.connections())
        )
...
class PipeGridEntry(GridEntry):
    ...
    @override
    def connections(self) -> Iterable[GridEntry]:
        return (c for delta in self.pipe.relative_connections() if (c := self.grid.get(self.pos + delta)) is not None)
...
def strictly_two[T](i: Iterable[T]) -> tuple[T, T]:
    first, second = mi.strictly_n(i, 2)
    return first, second
