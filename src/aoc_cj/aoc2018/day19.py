...
def part_1(txt: str, *, part: Literal[1, 2] = 1) -> int:
    ...
    ip = int(ip_s[4:])
    instructions = [day16.Instruction.parse_str(line) for line in instructions_strs]

    registers: day16.Registers = (0 if part == 1 else 1, 0, 0, 0, 0, 0)
    ...
    while registers[ip] < len(instructions) and remaining > 0:
        ...
        registers = day16.run_cmd(opcode, registers, instruction)
        registers = day16.update_registers(registers, ip, registers[ip] + 1)
        remaining -= 1
