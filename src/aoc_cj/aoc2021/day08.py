...
def get_output_for_line(line: str) -> int:
    ...
    groups = mi.bucket(patterns, key=len)
    patterns_by_length = {length: {"".join(sorted(pattern)) for pattern in groups[length]} for length in groups}

    one_pattern = mi.one(patterns_by_length[SEGMENTS_IN_ONE])  # 1 is the only digit that uses 2 segments
    four_pattern = mi.one(patterns_by_length[SEGMENTS_IN_FOUR])  # 4 is the only digit that uses 4 segments
    seven_pattern = mi.one(patterns_by_length[SEGMENTS_IN_SEVEN])  # 7 is the only digit that uses 3 segments
    ...
    three_pattern = mi.one(p for p in patterns_by_length[5] if all(x in p for x in one_segments))
    pattern_to_digit[three_pattern] = 3

    # 6 is the only digit that uses 6 segments and only one of the segments in digit 1
    six_pattern = mi.one(p for p in patterns_by_length[6] if sum(x in p for x in one_segments) == 1)
    pattern_to_digit[six_pattern] = 6

    # 2 is the only digit that uses 5 segments and has 2 segments in common w/ 4
    two_pattern = mi.one(p for p in patterns_by_length[5] if sum(x in p for x in four_segments) == 2)
    pattern_to_digit[two_pattern] = 2

    # 5 is the only remaining digit that uses 5 segments
    five_pattern = mi.one(p for p in patterns_by_length[5] if p not in (two_pattern, three_pattern))
    pattern_to_digit[five_pattern] = 5

    # 9 is the only digit that uses 6 segments and has 4 segments in common w/ 4
    nine_pattern = mi.one(p for p in patterns_by_length[6] if sum(x in p for x in four_segments) == 4)
    pattern_to_digit[nine_pattern] = 9

    # 0 is the only remaining digit that uses 6 segmensts
    zero_pattern = mi.one(p for p in patterns_by_length[6] if p not in (six_pattern, nine_pattern))
